...
## Step 0 — 脚手架
- [ ] 建 `packages/catalog`,照 core 拷配置;`name` = `@function-renderer/catalog`。
- [ ] `dependencies`:`@function-renderer/core` `workspace:*`、`zod` `catalog:`;`devDependencies` 加 `@function-renderer/runner` `workspace:*`(测试)+ 标准 devDeps。
- [ ] `vp install`。
...
- [ ] `tests/examples.test.ts`:用 `@function-renderer/runner` 跑每个示例,断言 `result`/`state`(AC3)。

## Step 3 — 校验
- [ ] `vp run -r build`;`vp run --filter @function-renderer/catalog test` 绿;`vp check packages/catalog` 干净(必要时 `--fix`)(AC4)。
...
vp run --filter @function-renderer/catalog test
