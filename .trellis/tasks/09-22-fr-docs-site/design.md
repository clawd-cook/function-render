...
## 包:`packages/leetcode`(`@function-renderer/leetcode`)
- deps:`@function-renderer/core`(类型)、`@function-renderer/runner`(测试/运行)、`zod`。
- `src/problems/<slug>.ts`:每题一个模块,导出 `Problem`:
  ...
  interface Problem {
    ...
    spec: Node;                 // function-render 实现(调用注册函数)
...
- `docs` 根 = `apps/docs`;`.vitepress/config.ts`:title/description、`base: "/function-render/"`、nav、sidebar(介绍 + LeetCode 分类,由 leetcode 包数据生成)。
- 介绍页:`index.md`(首页 hero)、`guide/overview.md`、`guide/operators.md`、`guide/api.md`(概念/算子/API,摘自现有 README)。
- LeetCode:动态路由 `leetcode/[slug].md` + `leetcode/[slug].paths.js`(从 `@function-renderer/leetcode` 的 `problems` 生成 params:title/num/url/difficulty/description/python/spec 字符串)。页面模板渲染这些字段 + `<OnlineRunner :slug="..."/>`。
- `.vitepress/theme/`:注册全局组件 `OnlineRunner`。
- `leetcode/index.md`:分类目录表(链接到各题页)。

## 运行器组件:`OnlineRunner.vue`
- props:`slug`。从 `@function-renderer/leetcode` 按 slug 找到 Problem。
...
- VitePress `base: "/function-render/"`(仓库名 function-render → 项目页 URL `https://clawd-cook.github.io/function-render/`)。

## 集成注意(已验证)
- `apps/docs` devDeps 必须含 `vue` + `esbuild`(否则 Vite+ core 下 VitePress 构建失败)。
- 运行器组件 import `@function-renderer/leetcode`(workspace),需先 `vp run -r build` 出 dist(库),VitePress 再构建(bundler 解析 dist)。CI 顺序同理。
