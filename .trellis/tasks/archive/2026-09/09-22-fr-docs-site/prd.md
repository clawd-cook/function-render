...
> 从最新 `main` 切分支。目标:打通「文档站 + LeetCode 题(logic-render 实现)+ 浏览器在线运行 + GitHub Pages 部署」完整流水线,首批若干题,后续可扩到全部 93。

## Goal

一个 VitePress 文档站:(1) 介绍 logic-render 项目(概念/算子/API);(2) LeetCode Hot100 分类目录 + 每题页(题目描述、原 Python 解法、logic-render spec、**浏览器在线运行器**);(3) GitHub Pages 自动部署。

数据来源:`https://github.com/realnghon/LeetCode_Hot100_Python`(公开题解,文档内注明来源)。

## Confirmed Facts / Decisions

- **落地方式(已认可)**:每题把 Python 解法移植成 TS 函数,放进新包 `@logic-renderer/leetcode`;该题「logic-render 实现」= 调用它的 spec(单 `call` 或用 `seq`/`for` 编排);在 VitePress 里嵌浏览器内运行器,复用 `@logic-renderer/runner`。
...
- **R1 leetcode 包**:`@logic-renderer/leetcode` 导出 `problems`:每题 `{ num, title, slug, url, difficulty, category, description, python, solution, spec, catalog, sample }`。`solution` = 移植的 TS 函数;`spec` = logic-render Node;`sample` = 默认输入 + 期望输出。
...
- **R5 部署**:`.github/workflows/deploy-docs.yml` 构建 VitePress 并部署 GitHub Pages;VitePress `base` 设为 `/logic-render/`(项目页)。
- **R6 验证**:`leetcode` 包单测(每题 solution + spec 经 runner 得期望);站点 `vitepress build` 成功;浏览器手测一题在线运行;部署 workflow 合理。

## Acceptance Criteria

- [ ] AC1 `@logic-renderer/leetcode` 首批题:每题 `solution(sample.input)` 与 `run(spec,{catalog, initialState})` 都得 `sample.expected`;单测绿。
...
- 运行器在浏览器内执行 `@logic-renderer/runner`(纯前端,无后端)。
