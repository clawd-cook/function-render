...
2. **文档直呈协议、可点击执行**:LeetCode 不再把「每题一个解法函数」收进 TS 包;每题在文档里**直接展示协议(spec JSON)**,可编辑并点击运行(复用 `@function-renderer/runner`)。
...
- Q3 协议存放:(a) docs 内数据(不建解法函数包)+ 小复杂算子 catalog;是否**移除**已合并的 `@function-renderer/leetcode`?
